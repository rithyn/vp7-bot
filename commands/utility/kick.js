...
module.exports = {
    ...
    async execute (interaction) {
        ...
        if (user === interaction.user) return await interaction.reply({ content: 'You can\'t kick yourself.', ephemeral: true });
        if (user === interaction.client.user) return await interaction.reply({ content: 'You can\'t kick me. :/', ephemeral
        : true});
        const embed = new EmbedBuilder()
					...
					.addFields(
						...
						{
						name: "Why ?",
						value: "You have not followed the server rules. A moderator has decided to kick you. If you think this decision is unfair, you can contact the moderation team and explain why. Try to follow the rules next time.",
						inline: false
						},
						{
